...
<body>
	...
	<section class="bannertop">
						...
						<ul>
							...
							<li>
								...
								<ul id="buttonUI2" class = button123 style="display: none;">
									...
									<li><a href="Servlet?command=notice">NOTICE</a>
									</li>
									<li><a href="Servlet?command=photo&category=all">PHOTO</a>
								</ul>
							</li>
						</ul>
					</div>
		...
		<section class="tabwrap">
			<ul class="tab">
				<li class="tab <%= category.equals("all") ? "on" : "" %>">
					<p>
						<a href="Servlet?command=photo&category=all"> <span>전체 </span></a>
					</p>
				</li>
				<li class="tab <%= category.equals("matches") ? "on" : "" %>"><a href="Servlet?command=photo&category=matches"><span>Matches</span></a></li>
				<li class="tab <%= category.equals("squad") ? "on" : "" %>"><a href="Servlet?command=photo&category=squad"><span>Squad</span></a></li>
				<li class="tab <%= category.equals("traning_rest") ? "on" : "" %>"><a href="Servlet?command=photo&category=traning_rest"><span>Training&Rest</span></a></li>
				<li class="tab <%= category.equals("historical_view") ? "on" : "" %>"><a href="Servlet?command=photo&category=historical_view"><span>Historical View</span></a></li>
				<li class="tab <%= category.equals("supporters") ? "on" : "" %>"><a href="Servlet?command=photo&category=supporters"><span>Supporters</span></a></li>
				<li class="tab <%= category.equals("fc_seoul_event") ? "on" : "" %>"><a href="Servlet?command=photo&category=fc_seoul_event"><span>FC Seoul Event</span></a></li>
				<li class="tab <%= category.equals("future_of_seoul") ? "on" : "" %>"><a href="Servlet?command=photo&category=future_of_seoul"><span>Future of Seoul</span></a></li>
			</ul>
		</section>
		<div class="ntwrap">
			<form action="Servlet">
			...
			<div class="plwrap">
				...
				%>
					<li><a
						href="Servlet?command=photoDetail&bno=<%=bno%>">
							<div class="imgwrap">
								<img src="<%=img%>">
							</div>
							<div class="txtwrap">
		...
		<div class="page">
			<a href="Servlet?command=photo&page=<%=pageNum - 1%>&category=<%=category%>&searchValue=${searchValue}"></a>
			<%
			if(pageNum <=10 && maxCount <= 10){
				for(int i=1; i<=maxCount; i++){
					if(pageNum == i){
			%>
				<a class="on" href="Servlet?command=photo&page=<%=i %>&category=<%=category%>&searchValue=${searchValue}"><%=i %></a>
			<%
				} else {
			%>
				<a href="Servlet?command=photo&page=<%=i%>&category=<%=category%>&searchValue=${searchValue}"><%=i %></a>
			<%
				}
			}
			...
			if(pageNum == maxCount){
			%>
			<a href="Servlet?command=photo&page=<%=maxCount%>&category=<%=category%>&searchValue=${searchValue}"></a>
			<%
			} else {
			%>
			<a href="Servlet?command=photo&page=<%=pageNum + 1%>&category=<%=category%>&searchValue=${searchValue}"></a>
